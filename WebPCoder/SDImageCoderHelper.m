...
+ (void)animatedImage:(UIImage *)animatedImage count:(NSUInteger *)count duration:(NSTimeInterval *)duration {
    ...
    if ([imageRep isKindOfClass:[SDAnimatedImageRep class]]) {
        ...
        if (animatedImageRep.frames) {
            *count = animatedImageRep.frames.count;
            *duration = animatedImageRep.frames[0].duration;
            return;
        }
    }
    ...
    *count = [[bitmapImageRep valueForProperty:NSImageFrameCount] unsignedIntegerValue];
    if (*count == 0) {
        return;
    }
    [bitmapImageRep setProperty:NSImageCurrentFrame withValue:@(0)];
    *duration =  [[bitmapImageRep valueForProperty:NSImageCurrentFrameDuration] doubleValue];
}

+ (CGImageRef)imageFromAnimatedImage:(UIImage *)animatedImage index:(NSInteger)index duration:(NSTimeInterval *)duration {
    NSRect imageRect = NSMakeRect(0, 0, animatedImage.size.width, animatedImage.size.height);
    NSImageRep *imageRep = [animatedImage bestRepresentationForRect:imageRect context:nil hints:nil];
    // Check weak assigned frames firstly
    if ([imageRep isKindOfClass:[SDAnimatedImageRep class]]) {
        SDAnimatedImageRep *animatedImageRep = (SDAnimatedImageRep *)imageRep;
        if (animatedImageRep.frames) {
            if (index >= animatedImageRep.frames.count) {
                return nil;
            }
            SDImageFrame *frame = animatedImageRep.frames[index];
    ...
    NSInteger count = [[bitmapImageRep valueForProperty:NSImageFrameCount] unsignedIntegerValue];
    if (count == 0 || index >= (count)) {
        return nil;
    }
    [bitmapImageRep setProperty:NSImageCurrentFrame withValue:@(index)];
    *duration =  [[bitmapImageRep valueForProperty:NSImageCurrentFrameDuration] doubleValue];
