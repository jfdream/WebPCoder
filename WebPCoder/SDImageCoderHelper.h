...
#import <WebPCoder/SDImageFrame.h>
...
/// 获取动图的图片总张和总时长
/// @param animatedImage 动图图片
/// @param count 图片总张数
/// @param duration 单张总时长
+ (void)animatedImage:(UIImage * _Nullable)animatedImage count:(NSUInteger * _Nullable)count duration:(NSTimeInterval * _Nullable)duration;

/// 获取动图中的单张图片
/// @param animatedImage 动图图片
/// @param index 图片位置
/// @param duration 单张图片的显示时长
+ (CGImageRef _Nullable)imageFromAnimatedImage:(UIImage * _Nullable)animatedImage index:(NSInteger)index duration:(NSTimeInterval * _Nullable)duration;
